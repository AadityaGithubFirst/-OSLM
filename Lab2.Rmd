...
plot(0:20, y<-z, pch=19)
z<-dbinom(0:20, size = 20, prob = 0.1)
plot(x<-0:20, y<-z, pch=19)
z<-dbinom(0:20, size = 20, prob = 0.9)
plot(x<-0:20, y<-z, pch=19)
```
...
```{r}
lam2<-dpois(0:20, 2)
plot(0:20, lam2, pch=19)
lam10<-dpois(0:20, 10)
plot(0:20, lam10, pch=19)
```
...
?rnorm
set.seed(100)
sdnormhund<- rnorm(100, mean = 10, sd = 2)
mean(sdnormhund)
sd(sdnormhund)
median(sdnormhund)
summary(sdnormhund)
...
```{r}
alpha <- 0.05
f <- function(mu, x_bar, q) {
  pnorm(q, mean = mu, sd = 0.2) - (1 - q)
}
lower <- uniroot(f, interval = c(-100, 100), x_bar = x_bar, q = 1-alpha/2)$root
...
library(BSDA)
z.test(sdnormhund,sigma.x=2)
z_test <- z.test(sdnormhund, sigma.x=2)
...
uniroot(f,interval=c(-100,100),x.bar=mean(sdnormhund),s=sd(sdnormhund),n=length(sdnormhund),q=alpha)$root
...
f <- function(lambda, x.sum) {
  -dpois(x.sum, 10 * lambda, log = TRUE)
}
x_sum <- sum(x)

# Calculate the confidence interval
lower <- uniroot(f, interval = c(0, 1000), x.sum = x_sum)$root / 10
upper <- uniroot(f, interval = c(lower, 100), x.sum = x.sum, extend = "upX")$root / 10

# Print the confidence interval
cat("95% confidence interval for lambda:", lower, "-", upper, "\n")
# FILL ME IN
```
