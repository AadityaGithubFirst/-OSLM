...
```{r}
field<-setdiff(names(df), c("Gender", "Drugs", "Complications"))
...
```
```{r}
hist(log10(df$Cost),col="darkseagreen2")
df %>% filter(.,Cost>0) -> df
df$Cost = log10(df$Cost)
```
...
```{r}
gdc<-setdiff(names(df), field)
df.gdc <- gather(select(df, gdc))
df.gdc %>% select(key,value) %>% table(.)
ggplot(data=df.gdc,mapping=aes(x=value)) +
...
```{r}
ggplot(data=df.gdc,mapping=aes(x=value,y=rep(df$Cost,3))) +
  geom_boxplot(fill="darkorange1") +
  xlab("Values") + ylab("Log Cost")+
  facet_wrap(~key,scales='free')
```
...
```{r}
q<-setdiff(field, "Cost")
df.quant<-gather(select(df, q))
ggplot(data=df.quant,mapping=aes(x=value,y=rep(df$Cost, length(q)))) +
  geom_point() +
  xlab("Values") + ylab("Log Cost")+
  facet_wrap(~key,scales='free')
```
