...
```
The Euclidean distance will be euivalent to the following matrix
[[0, 1, 2], [sqrt(3), 0], [sqrt(12), sqrt(3)]]
```
...
df<-filter(df, dec>56&ra<264&v>-350&v< -250)
gr<-(df$g - df$r)
ri<-(df$r-df$i)
df.new<-df %>%mutate(gr, ri)
```
...
df.newer<-scale(df.new)
set.seed(25)
km2.out<-kmeans(df.newer, 2, nstart = 20)
ggpairs(df.new,progress=FALSE,mapping=aes(color=factor(km2.out$cluster)))
```
...
```
```
There are two clusters of sizes 686 and 532. the ratio of the between-cluster means of square and the total sum of square is 29.5%.
```
...
for ( ii in 2:10 ) {
  set.seed(25)
  km.out <- kmeans(df.newer,ii,nstart=20)
...
#From the silhouette method it is seen that the value for k for which the value produced by the silhouette method is the highest is at the point at which k=2 so taking the value of k=2
```
...
```{r}
rm(df.new, df.newer, df.plot, km.out, km2.out, gr, ii, ri, ss)
df<-read.csv("diamonds.csv", stringsAsFactors = TRUE)
df.new <- df %>% mutate(X=NULL, cut=NULL, color=NULL, clarity=NULL)
```
...
gs <- clusGap(scale(df.small),FUN=kmeans,nstart=20,K.max=8,B=50)
fviz_gap_stat(gs)
```
...
```{r}
km3.out<-kmeans(df.small, 3, nstart = 20)
...
df.small<-df %>% slice(., s)
validation_kproto(data = df.small, k = 2:5, nstart = 5, verbose = FALSE)
```
```
From the output it can be examined that the cluster with the highest value of indices is number two so that means that the ideal number of clusters is 2.
```
